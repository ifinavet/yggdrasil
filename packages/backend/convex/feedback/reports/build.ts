...
export async function prepareReport(ctx: MutationCtx, campaignId: Id<"feedbackCampaigns">) {
	...
	const existing = await ctx.db.query("feedbackReports").withIndex("by_campaignId", (index) => index.eq("campaignId", campaignId)).unique();
	if (existing) return existing._id;
	const campaign = await ctx.db.get(campaignId);
	if (!campaign || campaign.status !== "closed" || !campaign.formVersionId || campaign.retainedAt !== undefined)
	...
	const retentionAt = campaign.retentionAt ?? feedbackRetentionAt(campaign.closedAt ?? campaign.closesAt);
	...
	const [logo, application, storedFields] = await Promise.all([
		...
		ctx.db.query("companyApplications").withIndex("by_eventId", (index) => index.eq("eventId", event._id)).first(),
		ctx.db.query("formFields").withIndex("by_formVersionId_and_order", (index) => index.eq("formVersionId", formVersionId)).take(41),
	]);
	...
	const reportId = await ctx.db.insert("feedbackReports", {
		campaignId, eventId: event._id, eventTitle: event.title, eventStart: event.eventStart,
		companyName: company.name, companyLogoId: logo?.image,
		recipientEmail: application?.contact.email ?? "", status: "building",
		questions: createReportQuestions(fields.data), totalResponses: 0,
		buildCursor: null, revision: 0, retentionAt,
	});
	await ctx.scheduler.runAfter(0, internal.feedback.reports.build.buildReportBatch, { reportId, cursor: null });
	return reportId;
}
...
export const buildReportBatch = internalMutation({
	...
	handler: async (ctx, { reportId, cursor }) => {
		...
		if (!report || report.status !== "building" || report.buildCursor !== cursor) return;
		const responses = await ctx.db.query("formResponses")
		...
		for (const response of responses.page) {
			...
			for (const answer of textAnswers)
				await ctx.db.insert("feedbackReportAnswers", { reportId, responseId: response._id, ...answer, visible: true });
			totalResponses += 1;
		}
		await ctx.db.patch(reportId, { questions: report.questions, totalResponses,
			buildCursor: responses.continueCursor, status: responses.isDone ? "draft" : "building" });
		if (!responses.isDone) await ctx.scheduler.runAfter(0, internal.feedback.reports.build.buildReportBatch, { reportId, cursor: responses.continueCursor });
	},
});
