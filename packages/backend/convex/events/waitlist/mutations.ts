...
import { tracedInternalMutation } from "../../lib/trace";
 ...
 */
export const checkPendingRegistrations = tracedInternalMutation({
	handler: async (ctx) => {
 ...
 */
export const clearWaitlistAndPending = tracedInternalMutation({
	handler: async (ctx) => {
		const now = Date.now();
		...
		ctx.trace("clear_waitlist_scan", {
			eventCount: registrationsForEvents.length,
			registrationCount: registrationsForEvents.reduce(
 ...
 */
export const fixWaitlist = tracedInternalMutation({
	args: {
		eventId: v.id("events"),
	},
	handler: async (ctx, { eventId }) => {
		...
		ctx.trace("fix_waitlist_scan", {
			eventId,
			registeredAndPending: registrations.length + pending.length,
