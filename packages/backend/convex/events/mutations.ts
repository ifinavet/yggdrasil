...
import { type TracedMutationCtx, tracedInternalMutation, tracedMutation } from "../lib/trace";
 ...
 */
export const update = tracedMutation({
	args: {
		id: v.id("events"),
		title: v.string(),
		teaser: v.string(),
	...
	handler: async (
		...
		if (participationLimit - event.participationLimit > 0 && waitlistLength) {
			ctx.trace("event_capacity_increased", {
				eventId: event._id,
				newPlaces: participationLimit - event.participationLimit,
			});
 ...
 */
export const updateWaitlistMutation = tracedInternalMutation({
	args: {
		eventId: v.id("events"),
