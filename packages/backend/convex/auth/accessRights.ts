...
/**
 * Refuses a change that would leave the organization without a super-admin.
 *
 * @param {MutationCtx} ctx - The Convex mutation context.
 * @param {Id<"users">} userId - The id of the user whose role is changing.
 * @param {AccessRole | null} newRole - The role the user is getting, or null when it is revoked.
 *
 * @throws - An error if the user is the last super-admin and would lose that role.
 * @returns {Promise<void>} - Resolves when the change is allowed.
 */
async function requireAnotherSuperAdminRemains(
	ctx: MutationCtx,
	userId: Id<"users">,
	...
	if (superAdmins.length <= 1) {
		throw new ConvexError("Kan ikke fjerne den siste super-administratoren.");
	}
}
...
export async function assignAccessRole(
	...
	await requireAnotherSuperAdminRemains(ctx, userId, role);
...
export async function revokeAccessRole(ctx: MutationCtx, userId: Id<"users">): Promise<void> {
	await requireAnotherSuperAdminRemains(ctx, userId, null);
