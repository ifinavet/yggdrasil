	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			try {
				...
				toast.error(
					error instanceof ConvexError ? String(error.data) : "Kunne ikke lagre annonsen.",
				);
			}
		},
	});
		...
		>
			<FieldGroup>
				<form.Field name="title">
					{(field) => (
						<LabeledField field={field} label="Tittel">
				...
				<form.Field name="teaser">
					{(field) => (
						<LabeledField field={field} label="Intro">
							<TextInput field={field} />
						</LabeledField>
					)}
				</form.Field>
				<form.Field name="description">
					{(field) => (
						<LabeledField field={field} label="Beskrivelse">
							<RichTextEditor
								id={field.name}
								value={field.state.value}
				...
				<form.Field name="applicationUrl">
					{(field) => (
						<LabeledField field={field} label="Søknadslenke">
				...
				<div className="grid gap-6 sm:grid-cols-2">
					<form.Field name="deadline">
						{(field) => (
							<LabeledField field={field} label="Søknadsfrist">
					...
					<form.Field name="type">
						{(field) => (
							<LabeledField field={field} label="Ansettelsesform">
