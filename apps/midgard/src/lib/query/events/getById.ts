...
export async function getEventById(id: number) {
  ...
  const {
    data: registrations,
    error: registrations_error,
  } = await supabase.from("registrations").select("*").eq("event_id", id);
  ...
  const registrationsByStatus =
    registrations?.reduce((acc: Record<string, Tables<"registrations">[]>, registration: Tables<"registrations">) => {
