...
export default async function Layout({
  ...
  const { data: internal_member, error: internal_member_error } = await supabase
  ...
  if (internal_member == null && internal_member_error != null) {
    console.log("Error fetching internal member: ", internal_member_error);
    return redirect("/sign-in");
  }

  const imageUrl = await getInternalMemberImage(
    internal_member?.profile_image_id ?? "",
  );
