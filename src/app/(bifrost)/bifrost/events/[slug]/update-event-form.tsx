...
import { type EventFormValues, formSchema } from "@/utils/bifrost/schemas/event-form-schema";
...
export default function UpdateEventForm({ event_id, orgId }: { event_id: number; orgId: string }) {
	...
	const [selectedOrganizerType, setSelectedOrganizerType] = useState<"main" | "assistant">(
		"assistant",
	);

	const [selectedOrganizers, setSelectedOrganizers] = useState(event.organizers);
	...
	const handleRoleChange = (organizerId: string, newRole: "main" | "assistant") => {
	...
	const handleDeleteOrganizer = (organizerId: string) => {
		...
		setSelectedOrganizers((prev) => prev.filter((organizer) => organizer.id !== organizerId));
